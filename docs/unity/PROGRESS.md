...
| M12 — differential harness vs the JS | ◐ in progress | 2026-08-21 — the JS boots headlessly (31/31 scripts); golden traces with per-ply projection hashes; opening parity PROVEN on 3 pairings; replay agrees ply-by-ply through the turn machine, plays, builds, upgrades and upkeep settlement (54/62/31 plies). Remaining: the combat commands (declare/resolve/respond), which need the five JS suspension points scripted |
